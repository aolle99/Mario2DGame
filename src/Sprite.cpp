...
void Sprite::render(bool leftdir,bool star) const
{
	glm::mat4 modelview = glm::translate(glm::mat4(1.0f), glm::vec3(position.x, position.y, 0.f));
	//mirar hacia el otro lado
	if (leftdir) {
		modelview = glm::translate(modelview, glm::vec3(size.x/2.f, size.y/2.f, 0.f));
		modelview = glm::scale(modelview, glm::vec3(-1.f, 1.f, 1.f));
		modelview = glm::translate(modelview, glm::vec3(-1* (size.x / 2.f),-1* (size.y / 2.f), 0.f));
	}

	shaderProgram->setUniformMatrix4f("modelview", modelview);
	shaderProgram->setUniform2f("texCoordDispl", texCoordDispl.x, texCoordDispl.y);
	if (star) {
		float red = 0.5f + 0.5f * sin(starTime);
		float green = 0.5f + 0.5f * sin(starTime + 2.0f * 3.14159f / 3.0f);
		float blue = 0.5f + 0.5f * sin(starTime + 4.0f * 3.14159f / 3.0f);
		shaderProgram->setUniform4f("color", red, green, blue, 1.0f);
	}
	else {
